...
exports.reduce = exports.flatten = exports.pure = exports.liftF = exports.Step = exports.Pure = exports.Suspend = exports.Free = void 0;
...
class Suspend extends Free {
    ...
    chain(f) {
        return new Suspend(this.value.map((free) => free.chain(f)));
    }
    ap(f) {
        return this.chain(x => f.map(g => g(x)));
    }
    resume() {
        return (0, either_1.left)(this.value);
    }
    fold(f, g) {
        return g(this.value.map(free => free.fold(f, g)));
    }
    foldM(f, g) {
        return g(this.value).chain(free => free.foldM(f, g));
    }
    run(f) {
        let r = this.resume();
    ...
    eq(f) {
        let result = false;
        this.resume().takeLeft().map((func) => result = func.eq(f));
        return result;
    }
}
...
class Pure extends Free {
    ...
    resume() {
        return (0, either_1.right)(this.value);
    }
    fold(f, _) {
        return f(this.value);
    }
 ...
 */
const liftF = (f) => new Suspend(f.map(a => new Pure(a)));
exports.liftF = liftF;
/**
