...
describe("future", () => {
  describe("Future", () => {
    ...
    describe("fork", () => {
      ...
      it("should provide the final value", () =>
        new Promise<number>((res: any, rej: any) => inc(0).fork(rej, res)).then(
          (value: number) => assert(value).equal(1)
        ));

      it("should provide the error", () =>
        new Promise((res: any, rej: any) => err(msg).fork(rej, res)).catch(
          (e: Error) => assert(e.message).equal(msg)
        ));
  ...
  describe("wait", () => {
    it("should pause chain execution", async () => {
      ...
      return wait(1000).chain((): Future<void> => {
        ...
        if (x < 1000)
          return <Future<void>>(
            raise(new Error(`waited ${x} milliseconds ` + `instead of 1000`))!
          );
  ...
  describe("batch", () => {
    ...
    it("should succeed with all results", () => {
      ...
      return toPromise(
        ...
        .then((list: number[][]) =>
          assert(list).equate([[300, 600, 100], [200], [300, 500], [200, 600]])
        )
        .then(() =>
          assert(tags).equate(["a", "a", "a", "b", "c", "c", "d", "d"])
        );
    });
  ...
  describe("parallel", () => {
    ...
    it("should work with a list of raise values", () => {
      ...
      let par: Future<undefined[]> = parallel([raise(e), raise(e), raise(e)]);
