...
describe('file', () => {
        ...
        it('should provide absolute paths', () =>
            toPromise(
                statDirAbs(FIXTURES).map(m =>
                    assert(
                        reduce(m, true, (p, _, k) => (!p ? p : isAbsolute(k)))
                    ).true()
                )
            ));
    });
        ...
        it('should list absolute paths', () =>
            toPromise(
                listAbs(FIXTURES).map(m =>
                    assert(
                        m.reduce((p, c) => (!p ? p : isAbsolute(c)), true)
                    ).true()
                )
            ));
    });
        ...
        it('should list all the directories in a directory absolutely', () =>
            toPromise(
                listDirsAbs(FIXTURES).map(l =>
                    assert(
                        l.reduce((p, c) => (!p ? p : isAbsolute(c)), true)
                    ).true()
                )
            ));
    });
    ...
    describe('isDirectory', () => {
        it('should not fail if the directory does not exist', () =>
            toPromise(
                isDirectory(RANDOM_FILE).map(yes => assert(yes).be.false())
            ));
    });
    ...
    describe('copy', () => {
        ...
        it('should copy dirs recursively', () =>
            doFuture(function* () {
                ...
                let srcContents = yield readTextFile(
                    `${src}/dirab/dirabc/filea`
                );
                ...
                let destContents = yield readTextFile(
                    `${dest}/dirab/dirabc/filea`
                );
