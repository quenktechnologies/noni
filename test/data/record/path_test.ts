...
describe("path", () => {
  describe("tokenize", () => {
    ...
    it("should not nest brackets", () => {
      assert(tokenize("path\\[with nested]]")).equate(["path[with nested]]"]);
  ...
  describe("set", () => {
    ...
    it("should not mangle arrays", () => {
      assert(
        set("flag", true, <any>{ n: 1, b: { d: [1, 2, 3] }, items: [12] })
  ...
  describe("project", () => {
    ...
    it("should negate", () => {
      assert(project({ a: false, b: false, c: true, d: false }, src)).equate({
        c: [3],
      });
    });
