 ...
 * Contents of a file.
 */
export type Contents = string | DataView | TypedArray;
...
export const statDir = (path: Path): Future<StatsM> =>
  stat(path).chain((s) =>
    !s.isDirectory()
      ? pure({})
      : readdir(path).chain((list) =>
          parallel(list.map((l) => stat(`${path}/${l}`))).map((stats) =>
            stats.reduce((p, c, i) => {
              p[list[i]] = c;
...
export const statDirAbs = (path: Path): Future<StatsM> =>
  statDir(path).map((stats) =>
    reduce(stats, {}, (p: StatsM, c, k) => {
      p[`${path}/${k}`] = c;
      return p;
    })
  );
...
export const statDirRec = (path: Path): Future<StatsM> =>
  statDirAbs(path).chain((stats) =>
    parallel(
      reduce(stats, [], (p: Future<StatsM>[], c, k) =>
...
export const isDirectory = (path: Path): Future<boolean> =>
  exists(path).chain((yes) =>
    yes ? stat(path).map((s) => <boolean>s.isDirectory()) : pure(<boolean>false)
  );
...
export const isFile = (path: Path): Future<boolean> =>
  exists(path).chain((yes) =>
    yes ? stat(path).map((s) => <boolean>s.isFile()) : pure(<boolean>false)
  );
...
export const readFile = (
  ...
  fromCallback((cb) => fs.readFile(path, <object>options, cb));
...
export const listFiles = (path: Path): Future<string[]> =>
  statDir(path).map((stats) =>
    reduce(stats, [], (p: string[], c, k) => (c.isFile() ? p.concat(k) : p))
  );
 ...
 * path does not exist.
 */
export const makeDir = (path: Path, options: object = {}): Future<void> =>
  fromCallback((cb) =>
    fs.mkdir(path, <any>merge({ recursive: true }, options), cb)
  );
...
export const unlink = (path: Path): Future<void> =>
  isDirectory(path).chain((yes) => (yes ? removeDir(path) : removeFile(path)));
...
export const removeFile = (path: Path): Future<void> =>
  exists(path).chain((yes) =>
    yes ? fromCallback((cb) => fs.unlink(path, cb)) : pure((() => {})())
  );
...
export const copy = (src: Path, dest: Path): Future<number> =>
  doFuture(function* () {
    if (yield exists(dest))
      return raise(new Error(`io/file#copy: Path "${dest}" already exists!`));
