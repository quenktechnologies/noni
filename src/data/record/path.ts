...
import { Record, clone, reduce, merge, set as setKey, isRecord } from './';
...
export const tokenize = (str: Path): Token[] => {
    ...
    while (i < str.length) {
        ...
        if (curr === TOKEN_ESCAPE) {
            ...
            if (buf !== '') tokens.push(buf); //recognize a path and push a new token

            buf = '';
        } else if (
            curr === TOKEN_BRACKET_LEFT &&
            ...
            while (true) {
                ...
                if (
                    ...
                    if (buf !== '') tokens.push(buf); //save the previous path
                    ...
                    if (firstDot !== -1) {
                        ...
                        tokens.push(
                            `${buf}${TOKEN_BRACKET_LEFT}${firstDotBuf}`
                        );
                        buf = '';
                        break;
                    } else {
...
export const unsafeGet = <A>(path: Path, src: Record<A>): A => {
    if (src == null) return <any>undefined;
    ...
    reduce(r, <Record<A>>{}, (p, c, k) => {
        if (typeof c === 'object')
            p = setKey(
                p,
                escape(k),
                <A>(<any>escapeRecord(<Record<A>>(<any>c)))
            );
        else p = setKey(p, escape(k), c);
...
export const unescapeRecord = <A>(r: Record<A>): Record<A> =>
    reduce(r, <Record<any>>{}, (p, c, k) => {
        if (isRecord(c)) p = setKey(p, unescape(k), unescapeRecord(<any>c));
        else p = setKey(p, unescape(k), c);
...
const prefix = (pfix: string, key: string) =>
    pfix === '' ? escape(key) : `${pfix}.${escape(key)}`;
