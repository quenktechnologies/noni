 ...
 */
export type ForEachFunction<A, B> = (
  value: A,
  key: string,
  rec: Record<A>
) => B;
...
export const map = <A, B>(rec: Record<A>, f: MapFunc<A, B>): Record<B> =>
  keys(rec).reduce((p, k) => merge(p, set({}, k, f(rec[k], k, rec))), {});
...
export const reduce = <A, B>(
  ...
  accum: B,
  f: ReduceFunc<A, B>
): B => keys(rec).reduce((p, k) => f(p, rec[k], k), accum);
...
export const filter = <A>(rec: Record<A>, f: FilterFunc<A>): Record<A> =>
  keys(rec).reduce(
    (p, k) => (f(rec[k], k, rec) ? merge(p, set({}, k, rec[k])) : p),
    {}
  );
...
export const rmerge = <A, R extends Record<A>, B, S extends Record<B>>(
  left: R,
  right: S
): R & S => reduce(right, <any>left, deepMerge);
...
>(
  ...
  [s, t].reduce(
    (p: R & S & T, c: S | T) => reduce(<Record<A | B>>c, p, deepMerge),
    <any>r
  );
...
>(
  ...
  [s, t, u].reduce(
    (p: R & S & T & U, c: S | T | U) =>
      reduce(<Record<A | B | C | D>>c, p, deepMerge),
    <any>r
  );
...
>(
  ...
  [s, t, u, v].reduce(
    (p: R & S & T & U & V, c: S | T | U | V) =>
      reduce(<Record<A | B | C | D | E>>c, p, deepMerge),
    <any>r
  );
  ...
  isRecord(curr)
    ? merge(
        ...
        set(
          {},
          key,
          isRecord(pre[key]) ? rmerge(<any>pre[key], curr) : merge({}, curr)
        )
      )
...
export const clone = <A, R extends Record<A>>(r: R): R =>
  <R>(<any>reduce(r, <Record<A>>{}, (p, c, k) => {
    set(p, k, _clone(c));
    return p;
  }));

const _clone = (a: any): any => {
  if (isArray(a)) return a.map(_clone);
  else if (isRecord(a)) return clone(a);
  else return a;
};
...
export const set = <A, R extends Record<A>>(r: R, k: Key, value: A): R => {
  if (!isBadKey(k)) (<Record<A>>r)[k] = value;
 ...
 */
export const isBadKey = (key: string): boolean => badKeys.indexOf(key) !== -1;
...
export const make = <A>(init: Record<A> = {}): Record<A> => {
  ...
  for (let key in init) if (init.hasOwnProperty(key)) rec[key] = init[key];
...
export const isEqual = (rec1: object, rec2: object): boolean =>
  keys(rec1).every(
    (key) => (<Record<Type>>rec2)[key] === (<Record<Type>>rec1)[key]
  );
